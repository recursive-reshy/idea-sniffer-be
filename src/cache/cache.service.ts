...
export class CacheService implements OnModuleInit{
  ...
  private load(): void {
    ...
    for ( const file of files ) {
      try {
        ...
        this.seenIds.set( provider, new Set( data.seenIds ) )
  ...
  isNew( provider: string, id: string ): boolean {
    return !( this.seenIds.get( provider )?.has( id ) ?? false )
  }
  ...
  private async persist( provider: string ): Promise< void > {
    ...
    try {
      const data = JSON.stringify( { seenIds: Array.from( this.seenIds.get( provider ) || [] ) }, null, 2 )
      ...
      this.logger.log( `Cache for provider ${ provider } persisted successfully with ${ this.seenIds.get( provider )?.size || 0 } IDs` )
    } catch ( error ) {
  ...
  async markAsSeen( provider: string, id: string ): Promise< void > {
    if ( !this.seenIds.has( provider ) ) {
      this.seenIds.set( provider, new Set() )
    }

    this.seenIds.get( provider )!.add( id )
  ...
  async markSeenBatch( provider: string, ids: string[] ): Promise< void > {
    ...
    const providerCache = this.seenIds.get( provider )
